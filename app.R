...
ui<- navbarPage(tags$strong("ARMED CONFLICT DASHBOARD"), id="nav",
                ...
                tabPanel("About",img(src='IMG_2155.JPG', align = "left"),  
                         ...
                         fluidPage( theme = shinytheme("spacelab"), HTML(
                           paste(
                             ...
                             h6("This app is better viewed on google explorer"), '<br/>',
                             h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard"), '<br/>'
                             )),
                         ...
                         fluidPage( HTML(
                           paste(
                             ...
                             p("Data presented in our charts contain detailed information regarding armed conflict events occuring in select regions around the world. 
                               ...
                               For more information regarding variable selection and analysis, please refer to ", a("white paper", target="_blank",href="UN_White_Paper.pdf"), "."), '<br/>',
...
tabPanel("Data Visualizations",
         tabsetPanel(id = "tabSelected",
                                ...
                                sidebarLayout(
                                  sidebarPanel(
                                    ...
                                    selectizeInput("filter2",
                                                   "Select Measure(Y-axis)",
                                                   choices = colnames(dataset[,c(8:11, 14)]),
                                                   selected = "GDP"),
                                    dateRangeInput("date_range",
                                                   label = "Date Range (X-axis)",
                                                   min = "1960-01-01",
                                                   max = "2016-12-31",
                                                   start = "1960-01-01",
                                                   end = "2016-12-31",
                                                   format = "yyyy",
                                                   startview = "decade"),
                                    h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard"),
                                    h6("GDP: Per Capita GDP in USD"),
                                    h6("Mortality: Natural Death Rate in %"),
                                    h6("Fertility: Birth Rate in %"),
                                    h6("Population: Size of Polution in OOO's"), 
                                    h6("Total Violent Fatalities: Number of people died")
                     ...
                     tabPanel("Density Plot",
                              ...
                              h6("Caption1: Peak shows when most events occurred (we are assuming each record is a number of total events). ", align="center"),
                              h6("Caption2: X-axis represents Years, Y-axis represents Density. ", align="center"),
                              h6("Caption3: option available to select country and measure. ", align="center"),
                              fluidPage(
                                sidebarLayout(
                                  sidebarPanel(
                                    selectInput("filter4",
                     ...
                     tabPanel("Scatter Plot",
                              ...
                              h6("Caption1: Option available to filter by country, hover on icons to view x and y values, hover on line to view R value. ", align="center"),
                              h6("Caption2: X-axis is a log10() transformed value of the variable Total Violent Fatalities, and Y-axis has a list of variables with log10()transformed values.", align="center"),
                              h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard", align= "center"),
                              ...
                              fluidRow(
                                ...
                                column(4, offset = 1,
                                       selectInput('x', 'X-axis (log10() transformed value of the variable Total Violent Fatalities)', choice = "Total_Violent_fatalities", names(dataset)[[14]]),
                                       selectInput('y', 'Y-axis (list of variables with log10()transformed values)', choice= c("GDP", "Trade_Import", "Trade_Export"), names(dataset)[[8]])
                     ...
                     tabPanel("Correlation Matrix",
                              ...
                              h6("Caption: colors indication whether the correlation is positive, negation, high, medium, or low. hover in each cell to view exact R-square value.", align="center"),
                              h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard", align= "center"),
                              fluidPage(
                     ...
                     tabPanel("Box Plot",
                              ...
                              h6("Caption1: option available to filter by country, use legend icons to select/unselect event type, hover on box plot to view descriptive statistics.", align="center"),
                              h6("Caption2: X-axis are the list of variables and Y-axis is a log10() transformed value of the variable Fatalities.", align="center"),
                              h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard", align= "center"),
                              ...
                              fluidRow(
                                ...
                                column(4, offset = 1,
                                       selectInput('y1', 'X- axis', choice= c("Event_type","Weapon_Type", "Nationality_of_Target"), names(ships)[[6]]),
                                       selectInput('x1', 'Y- axis (No. of fatalities are log10() transformed value)', choice = "Fatalities", names(ships)[[16]])
                                        )
                              )
...
tabPanel("Combined data table",
         h3("Table Provides View of Entire Dataset for Data Exploration", align="center"),
         h6("Caption: option available to search, sort, filter by country, and filter by year. ", align="center"),
         fluidRow(
           column(3,
                  selectInput("country6", label = strong("Country"), c("All Countries"="", list('Afghanistan', 'Iraq', 'Syria', 'Yemen')), multiple=TRUE)
           )
                    ),
         fluidRow(
           column(1,
                  numericInput("min6", label = strong("Year (From)"), min = 1950, max = 2017, value = 1950)
           ),
           column(1,
                  numericInput("max6", label = strong("Year (To)"), min=1950, max=2017, value=2017)
           )
         ),
         h6("**Disclaimer** unknown values were imputed as zero values for better display of our dashboard"),
         hr(),
...
server <- shinyServer(function(session,input, output) {
 ...
 #hide map table
  ...
  ####Data explorer server part2###
  
  output$Combinedtable <- DT::renderDataTable({
    dataset %>%
      filter(
        is.null(input$country6) | Country %in% input$country6,
        Year >= input$min6,
        Year <= input$max6
      )
      #mutate(Action = paste('<a class="go-map" href="" data-lat="', Latitude, '" data-long="', Longitude, '"><i class="fa fa-crosshairs"></i></a>', sep=""))
  ...
  output$plot2 <- renderPlotly({
    g<- ggplot(dataset3(), aes_string(x = input$y1 , y = input$x1)) + 
      geom_boxplot(aes_string(fill= input$y1)) +
